...
export default function TransactionDetail() {
  ...
  return <div className="page narrow"><div className="page-heading"><div><p className="eyebrow">Transaction Detail</p><h1 className="mono">{tx.transaction_id}</h1></div><Link className="btn btn-ghost" to="/transactions">← Back</Link></div>
    <section className="panel detail-grid">
      <div><span>Sender</span><strong>{tx.sender}</strong></div><div><span>Receiver</span><strong>{tx.receiver}</strong></div><div><span>Amount</span><strong>{Number(tx.amount).toLocaleString('vi-VN')} VND</strong></div><div><span>Status</span><strong><StatusBadge status={tx.status}/></strong></div><div><span>Timestamp</span><strong>{new Date(tx.timestamp).toLocaleString('vi-VN')}</strong></div><div><span>Nonce</span><strong className="mono small-text">{tx.nonce || '—'}</strong></div>
    </section>
