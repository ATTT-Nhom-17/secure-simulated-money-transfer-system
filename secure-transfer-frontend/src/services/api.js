  ...
  transactions: [
    {
      ...
      nonce: "NONCE001",
      signature: "DEMO_RSA_SIGNATURE_TX001",
      hash: "SHA256_DEMO_HASH_TX001",
    },
    {
      ...
      replay_detected: false,
      nonce: "NONCE002",
      signature: "DEMO_RSA_SIGNATURE_TX002",
      hash: "SHA256_DEMO_HASH_TX002",
    },
    {
...
export async function login(credentials) {
  ...
  if (credentials.username !== "user1" || credentials.password !== "123456") {
    throw new Error(
      "Invalid username or password. Demo account: user1 / 123456",
    );
  }
  localStorage.setItem("token", "demo-token");
  localStorage.setItem(
    "user",
    JSON.stringify({ username: "user1", email: "user1@example.com" }),
  );
...
export async function transfer(payload) {
  ...
  if (!/^\d{6}$/.test(String(payload.pin ?? ""))) {
    throw new Error("A 6-digit transfer PIN is required.");
  }

  if (payload.pin !== (state.transferPin || "123456")) {
    throw new Error("Invalid transfer PIN.");
  }
  if (amount > state.balance) throw new Error("Insufficient balance.");
  if (payload.receiver === state.username)
...
export async function getTransaction(id) {
  if (!USE_MOCK)
    return (await client.get(`/transactions/${encodeURIComponent(id)}`)).data;
  await delay();
